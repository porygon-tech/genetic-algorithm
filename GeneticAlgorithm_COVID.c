...
/*DEBUGGING:  pra
gcc GeneticAlgorithm_COVID.c RKF78-2.2.c/RKF78.c -o evo -lm -O3 -fsanitize=address -static-libasan -g -Wall && time ./evo
*/
...
individual* FindFittest(individual* ind) {
   individual *fittest = ind;
   for(int i=1; i < POPSIZE; i++) {
      if(ind[i].fitness < fittest -> fitness) fittest = ind+i;
   } 
   return fittest;
}
...
void mutateIndiv(individual *ind, double mu){
     mutate2(ind -> Pars[0], mu, 40);
     mutate2(ind -> Pars[9], mu, 40);
     mutate2(ind -> Pars[10], mu, 40);
     mutate2(ind -> Pars[1], mu, 20);
     mutate2(ind -> Pars[4], mu, 20);
     mutate2(ind -> Pars[5], mu, 20);
     mutate2(ind -> Pars[6], mu, 20);
     mutate2(ind -> Pars[8], mu, 20);
     mutate2(ind -> Pars[2], mu, 10);
     mutate2(ind -> Pars[3], mu, 10);
     mutate2(ind -> Pars[7], mu, 10);
     
}

void mating(individual *a, individual *b, individual *c, individual *d, double mu){
     memcpy(c, a, sizeof(individual));
     memcpy(d, b, sizeof(individual));
     crossover(c -> Pars[0], d -> Pars[0], 40);
     crossover(c -> Pars[9], d -> Pars[9], 40);
     crossover(c -> Pars[10], d -> Pars[10], 40);
     crossover(c -> Pars[1], d -> Pars[1], 20);
     crossover(c -> Pars[4], d -> Pars[4], 20);
     crossover(c -> Pars[5], d -> Pars[5], 20);
     crossover(c -> Pars[6], d -> Pars[6], 20);
     crossover(c -> Pars[8], d -> Pars[8], 20);
     crossover(c -> Pars[2], d -> Pars[2], 10);
     crossover(c -> Pars[3], d -> Pars[3], 10);
     crossover(c -> Pars[7], d -> Pars[7], 10);
     mutateIndiv(&c, mu);
     mutateIndiv(&d, mu);                                       
}
...
int main(int argc, char const *argv[])
{
	...
	printf("The fitness value of the 33th individual is: %f\n", population[33].fitness);

        individual* fittest = FindFittest(population);        
        printf("The best fitness is: %f\n", fittest -> fitness);
        
        double MU = 0.1; //prob mutation
       /*
        ...
        while(iter < max_iter){
          ...
          for (int j=0; j<POPSIZE/2; j++){
           ...
           // Crossover & Mutation
             memcpy(nextGen[j*2], p1, sizeof(individual));
	     memcpy(nextGen[j*2+1], p2, sizeof(individual));
	     
	     unsigned long auxf1 = nextGen[j*2] -> Pars[0], auxf2 = nextGen[j*2+1] -> Pars[0];
	     crossover(auxf1, auxf2, 40);
	     mutate2(auxf1, MU, 40); mutate2(auxf2, MU, 40);
	     nextGen[j*2] -> Pars[0] = auxf1; nextGen[j*2+1] -> Pars[0] = auxf2;
	     	     
	     auxf1 = nextGen[j*2] -> Pars[9]; auxf2 = nextGen[j*2+1] -> Pars[9];
	     crossover(auxf1, auxf2, 40);
	     mutate2(auxf1, MU, 40); mutate2(auxf2, MU, 40);
	     nextGen[j*2] -> Pars[9] = auxf1; nextGen[j*2+1] -> Pars[9] = auxf2;
	     
	     auxf1 = nextGen[j*2] -> Pars[10]; auxf2 = nextGen[j*2+1] -> Pars[10];
	     crossover(auxf1, auxf2, 40);
	     mutate2(auxf1, MU, 20); mutate2(auxf2, MU, 20);
	     nextGen[j*2] -> Pars[10] = auxf1; nextGen[j*2+1] -> Pars[10] = auxf2;
	     
	     auxf1 = nextGen[j*2] -> Pars[1]; auxf2 = nextGen[j*2+1] -> Pars[1];
	     crossover(auxf1, auxf2, 20);
	     mutate2(auxf1, MU, 20); mutate2(auxf2, MU, 20);
	     nextGen[j*2] -> Pars[1] = auxf1; nextGen[j*2+1] -> Pars[1] = auxf2;
	     
	     auxf1 = f1 -> Pars[4]; auxf2 = nextGen[j*2+1] -> Pars[4];
	     crossover(&auxf1, &auxf2, 20);
	     mutate2(&auxf1, MU, 20); mutate2(&auxf2, MU, 20);
	     f1 -> Pars[4] = auxf1; nextGen[j*2+1] -> Pars[4] = auxf2;
	     
	     auxf1 = nextGen[j*2] -> Pars[5]; auxf2 = nextGen[j*2+1] -> Pars[5];
	     crossover(&auxf1, &auxf2, 20);
	     mutate2(&auxf1, MU, 20); mutate2(&auxf2, MU, 20);
	     nextGen[j*2] -> Pars[5] = auxf1; nextGen[j*2+1]-> Pars[5] = auxf2;
	     
	     auxf1 = nextGen[j*2] -> Pars[6]; auxf2 = nextGen[j*2+1] -> Pars[6];
	     crossover(&auxf1, &auxf2, 20);
	     mutate2(&auxf1, MU, 20); mutate2(&auxf2, MU, 20);
	     nextGen[j*2] -> Pars[6] = auxf1; nextGen[j*2+1] -> Pars[6] = auxf2;
	     
	     auxf1 = nextGen[j*2] -> Pars[8]; auxf2 = nextGen[j*2+1] -> Pars[8];
	     crossover(&auxf1, &auxf2, 20);
	     mutate2(&auxf1, MU, 20); mutate2(&auxf2, MU, 20);
	     nextGen[j*2] -> Pars[8] = auxf1; nextGen[j*2+1] -> Pars[8] = auxf2;
	     
	     auxf1 = nextGen[j*2] -> Pars[2]; auxf2 = nextGen[j*2+1] -> Pars[2];
	     crossover(&auxf1, &auxf2, 10);
	     mutate2(&auxf1, MU, 10); mutate2(&auxf2, MU, 10);
	     nextGen[j*2] -> Pars[2] = auxf1; nextGen[j*2+1] -> Pars[2] = auxf2;
	     
	     auxf1 = nextGen[j*2]-> Pars[3]; auxf2 = nextGen[j*2+1] -> Pars[3];
	     crossover(&auxf1, &auxf2, 10);
	     mutate2(&auxf1, MU, 10); mutate2(&auxf2, MU, 10);
	     nextGen[j*2] -> Pars[3] = auxf1; nextGen[j*2+1] -> Pars[3] = auxf2;
	     
	     auxf1 = nextGen[j*2] -> Pars[7]; auxf2 = nextGen[j*2+1] -> Pars[7];
	     crossover(&auxf1, &auxf2, 10);
	     mutate2(&auxf1, MU, 10); mutate2(&auxf2, MU, 10);
	     nextGen[j*2] -> Pars[7] = auxf1; nextGen[j*2+1] -> Pars[7] = auxf2;
	      
             CoreModelVersusDataQuadraticError(&nextGen[2*j], &realData);
             CoreModelVersusDataQuadraticError(&nextGen[2*j+1], &realData);
          }
          
          individual* newFittest = FindFittest(nextGen); 
          if( newFittest.fitness < fittest.fitness ) iter ++;
          
          individual *temporary = population;
          population = nextGen;
          free (temporary);
