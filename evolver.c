...
#define POPSIZE 2000 // ((La que recomienda Alseda))
...
typedef struct {
	unsigned long IC[IC_GENES_NUMBER];           // integrer
	unsigned long Pars[PARAMETERS_GENES_NUMBER]; // integrer
	double DeltaPars[PARAMETERS_GENES_NUMBER];   // real numbers (truncation errors)
	double fitness;
} individual;


// Structure to store the ODE 11 params
typedef struct {
...
void CoreModelQuadraticErrorFitness(individual *ind, void *TheData_void) {
     DataForFitting *TheData = (DataForFitting *) TheData_void; 
     //Function that builds the IC for the differential equation
     double ic[CORE_MODEL_DIM] = { TheData -> PopSize,
                                   crom2IC(ind->IC[0]), crom2IC(ind->IC[1]), crom2IC(ind->IC[2]), 
	                           TheData -> Data_Time_Series[0][0],
	                           TheData -> Data_Time_Series[0][1],
	                           TheData -> Data_Time_Series[0][2],
	                           TheData -> Data_Time_Series[0][3] }, fitness_not_improved;
